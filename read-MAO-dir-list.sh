...
# 26 Sept 2023
#
# Script to read list of MAO directories ansd call prep-for-MAO-proc.sh for each line
# in given file.
...
do
	if [[ "$line" != "#"* ]]; then
		...
		./prep-and-download-for-MAO-proc.sh "${strarr[0]}" "${strarr[1]}" "${strarr[2]}" "${strarr[3]}" "${strarr[4]}"
	fi
done < "$input_file"
