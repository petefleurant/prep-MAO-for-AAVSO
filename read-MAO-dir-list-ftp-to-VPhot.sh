#!/bin/bash
#
# read-MAO-dir-list-ftp-to-VPhot.sh
#
# 28 Nov 2023
#
# Script to read list of MAO directories ansd call wrapper-ftp-to-VPhot.sh for each line
# in given file.
...
do
	if [[ "$line" != "#"* ]]; then
		...
		./wrapper-ftp-to-VPhot.sh "${strarr[0]}" "${strarr[1]}" "${strarr[2]}" "${strarr[3]}" "${strarr[4]}"
	fi
done < "$input_file"
