#!/bin/bash
#
# read-MAO-dir-list-calc-AIRMASS.sh
#
# 22 Nov 2023
#
# Script to read list of MAO directories ansd call wrapper-calc-AIRMASS.sh for each line
# in given file.
...
do
	if [[ "$line" != "#"* ]]; then
		...
		./wrapper-calc-AIRMASS.sh "${strarr[0]}" "${strarr[1]}" "${strarr[2]}" "${strarr[3]}" "${strarr[4]}"
	fi
done < "$input_file"
